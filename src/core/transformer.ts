...
export default function transformer(file: any, api: any, options: any) {
  ...
  root.find(j.TemplateLiteral).forEach((path: any) => {
      ...
      quasis.forEach((quasi: any, i: number) => {
      ...
      if (shouldIgnore(translationString.trim())) return;
      ...
      j(path).replaceWith(
          j.callExpression(j.identifier('t'), tArgs)
      );
      hasChanges = true;
  });
  ...
  if (hasChanges) {
    ...
    const hasI18nImport = imports.filter((p: any) => p.node.source.value.includes(i18n)).size() > 0;
    ...
    if (components.size() > 0) processComponent(components.at(0).get());
    else if (arrowComponents.size() > 0) processComponent(arrowComponents.at(0).get());
  }
